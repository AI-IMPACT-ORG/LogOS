...
HEADER_TITLE_LINE="LogOS: a host-minimal Agda library for modular dynamic logic systems synthesized by AI"
# shellcheck disable=SC2034
...
header_list_allowlisted_files() {
  ...
  # NOTE: `rg --files` applies globs in order; exclusions must come last so they
  # cannot be overridden by later allow globs.
