...
CHECK_NAME="doc-reference-check"
...
def resolve_repo_path(*, doc: Path, ref: str) -> Path | None:
    # Root-relative repo paths in inline code (`/docs/...`) are resolved
    # against the repo root for existence checking.
    if ref.startswith("/"):
...
def check_candidate(*, doc: Path, raw: str, kind: str, missing: list[str]) -> None:
    ...
    if re.search(r"\s", ref):
        ...
        return
    if kind == "link" and ref.startswith("/"):
        missing.append(
            f"{doc.relative_to(ROOT)}: root-relative markdown link: {raw!r}"
        )
        return
...
def main() -> int:
    ...
    for doc in docs:
        ...
        # Inline code spans outside markdown links.
        code_scan_text = LINK_DEST_RE.sub("", stripped)
        for span in CODE_SPAN_RE.findall(code_scan_text):
