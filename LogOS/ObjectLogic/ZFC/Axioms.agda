...
{-# OPTIONS --safe #-}
module LogOS.ObjectLogic.ZFC.Axioms where
...
module FromZFAxiomsᶠ {ℓ : Level}
  ...
  open Kernel K
  open ZFAxiomsᶠ zf
  open LogOS.ObjectLogic.ZFC.Signature.ForKernel K public
  ...
  Predᶠ : ∀ {n} → ZPredCode → FOL.Term n → FOL.Fml ΣZ n
  Predᶠ φ x = FOL.pred φ x

  Relᶠ : ∀ {n} → ZRelCode → FOL.Term n → FOL.Term n → FOL.Fml ΣZ n
  ...
  PredI : ZPredCode → SetU → Set ℓ
  PredI φ x = Pred (unPred φ) x

  RelI : ZRel₂ → SetU → SetU → Set ℓ
  RelI mem       x y = x ∈ y
  RelI eq        x y = x ≈ y
  RelI (rel ψ)   x y = Rel (unRel ψ) x y

  module Sem = FOL.For {Σ₀ = ΣZ} SetU PredI RelI
  open Sem
  ...
  Separationᶠ : ZPredCode → FOL.Fml ΣZ 0
  Separationᶠ φ =
    FOL.All
      (FOL.Ex (FOL.All
        (FOL.Iff
  ...
  Functionalᶠ : ZRelCode → FOL.Fml ΣZ 0
  Functionalᶠ ψ =
    FOL.All (FOL.All (FOL.All
      (((Relᶠ ψ v2 v1) FOL.∧ (Relᶠ ψ v2 v0)) FOL.⇒ (v1 ≈ᶠ v0))))

  ReplacementBodyᶠ : ZRelCode → FOL.Fml ΣZ 0
  ReplacementBodyᶠ ψ =
    FOL.All
      (FOL.Ex (FOL.All
        (FOL.Iff
  ...
  Replacementᶠ : ZRelCode → FOL.Fml ΣZ 0
  Replacementᶠ ψ =
  ...
  valid-Separationᶠ φ env x =
    let y    = proj₁ (separationᶠ (unPred φ) x)
        spec = proj₂ (separationᶠ (unPred φ) x)
    in y , (λ z → (Prop.to (spec z) , Prop.from (spec z)))

  valid-ReplacementBodyᶠ
    : ∀ ψ → FunctionalRel (Rel (unRel ψ)) → ∀ env → Sat env (ReplacementBodyᶠ ψ)
  valid-ReplacementBodyᶠ ψ fun env x =
    let y    = proj₁ (replacementᶠ (unRel ψ) fun x)
        spec = proj₂ (replacementᶠ (unRel ψ) fun x)
    in y , (λ z → (Prop.to (spec z) , Prop.from (spec z)))

  functionalRel-from-Sat
    : ∀ ψ env → Sat env (Functionalᶠ ψ) → FunctionalRel (Rel (unRel ψ))
