...
open import Data.NatOrder using (_≤ℕ_; total≤ℕ; z≤n; s≤s)
...
OmegaCPO : ∀ {ℓ} → ConPoset ℓ → Set (lsuc ℓ)
...
ωChain-from-zero
  ...
  → (ch : ωChain CP)
  → (n : ℕ)
  → ConPoset._⊑_ CP (ωChain.seq ch zero) (ωChain.seq ch n)
ωChain-from-zero {CP = CP} ch zero = ConPoset.refl CP
...
ωChain-mono≤ℕ
  ...
  → {m n : ℕ}
  → m ≤ℕ n
  → ConPoset._⊑_ CP (ωChain.seq ch m) (ωChain.seq ch n)
...
ωChain-directed {CP = CP} ch =
  ...
  where
    ...
    ... | inj₁ i≤j = (j , (ωChain-mono≤ℕ ch i≤j , ConPoset.refl CP))
    ... | inj₂ j≤i = (i , (ConPoset.refl CP , ωChain-mono≤ℕ ch j≤i))
