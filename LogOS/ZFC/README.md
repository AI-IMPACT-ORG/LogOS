...
WFGraph ZF/ZFC Mechanisation (LogOS.ZFC)
========================================
...
- full Separation/Replacement can be obtained in two ways:
  - **directly** from WFGraph `sup` formation (textbook route, see `LogOS/ZFC/WFGraph/Textbook.agda`);
  - **indirectly** from *definable* schemata under explicit representability assumptions
    (see `LogOS/ZFC/SetTheory/FullUpgradeFromDefinable.agda`).

Core entrypoints
----------------

- `LogOS/ZFC/WFGraph/Model.agda` — builds a definable-ZF base pack over a WF-graph carrier
  (membership graph + `sup`), with explicit Extensionality/Powerset/Foundation structures.
- `LogOS/ZFC/WFGraph/ZFC.agda` — adds ω and produces definable-ZF+Infinity; optionally upgrades
  to full `LogOS.ZFC.SetTheory.Pack.ZFAxioms` given explicit representability assumptions.
- `LogOS/ZFC/WFGraph/Structure.agda` — bundle the WF-graph carrier assumptions into one record
  (`WFGraphStructure`) so the core development has explicit, minimal dependencies.
- `LogOS/ZFC/WFGraph/Surface.agda` — one-stop façade with several layers:
...
- Stage packs and `StageToCH`: `LogOS/ZFC/SetTheory/Cumulative.agda`
- `LogOS/ZFC/SetTheory/StageToCHFromHierarchy.agda` — generic adapter: any `CumulativeHierarchy K`
...
- `LogOS/ZFC/SetTheory/CumulativeSurface.agda` — `stageToSurface` promotes `StageToCH` to `ZFDsl`.
- `LogOS/ZFC/SetTheory/FromZFAxioms.agda` — converts `ZFAxioms K` to `CumulativeHierarchy K`.
- `LogOS/ZFC/SetTheory/Derived.agda` — small derived set constructors/lemmas over `ZFAxioms`
  (e.g. `singleton`, `union₂`).
- `LogOS/ZFC/SetTheory/FormulaDerived.agda` — the same style of small derived constructors/lemmas,
...
- Full ZF Separation/Replacement is available *conditionally* via the explicit
  representability assumptions in `LogOS/ZFC/SetTheory/FullUpgradeFromDefinable.agda`.
- Full ZF Separation/Replacement is also available *constructively* in the WFGraph route
  when `supN` formation is available (see `LogOS/ZFC/WFGraph/Textbook.agda`).
- Choice (AC) is **not derived**; `ZFCAxioms` is exposed as `ZF + AC` where `AC` is a
  separate, explicit witness (`LogOS/ZFC/SetTheory/ChoiceAxiom.agda`).
...
- `LogOS/ZFC/Supplementary/` (alternate routes, e.g. hereditarily-finite developments)
