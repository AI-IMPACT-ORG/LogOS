...
{-# OPTIONS --safe #-}
module LogOS.ZFC.WFGraph.Textbook where
...
open import LogOS.API.Kernel using (Kernel; kernelLike-fromKernel)

open import LogOS.ZFC.SetTheory.ChoiceAxiom as AC using (AxiomOfChoice)
open import LogOS.ZFC.SetTheory.Pack as Pack using (ZFAxioms; ZFCAxioms)
open import LogOS.ZFC.SetTheory.FromZFAxioms using (toCumulativeHierarchy)
open import LogOS.ZFC.SetTheory.LimitPack using (CumulativeHierarchy)
open import LogOS.ZFC.SetTheory.StageToCHFromHierarchy using (StageToCH-fromCH)
open import LogOS.ZFC.SetTheory.CumulativeSurface using (stageToSurface)
open import LogOS.ZFC.SetTheory.Dsl using (ZFDsl)
import LogOS.ZFC.SetTheory.Derived as Derived
import LogOS.ZFC.SetTheory.SchemaTheorems as Schema
...
module ForZFC {ℓ : Level} (W : WFGraphStructure ℓ) where
    ...
    where
      ...
      from z (u , (u∈x , img≈z)) =
        _↔_.from (mem-sup↔ₛ {I = Σ SetU (λ u0 → u0 ∈ x)}
                           ...
                           {y = z})
          ((u , u∈x)
          , ExtensionalityStructure.ext≡ Ext (proj₁ (F u)) z (mem-ext img≈z)
          )
  ...
  open ZFTextbook public
    hiding (SetU; _∈_; _≈_; refl≈; sym≈; trans≈; extensionality; mem-ext; separation; replacement)
