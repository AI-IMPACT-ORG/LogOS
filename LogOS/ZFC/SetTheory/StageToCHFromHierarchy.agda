...
{-# OPTIONS --safe #-}
module LogOS.ZFC.SetTheory.StageToCHFromHierarchy where
...
open import LogOS.API.Kernel.Endo using (Endo; Flow-Endo; Th⋆K; Th⋆≤FlowTh⋆; FlowTh⋆≤Th⋆; id≤Flow)
...
StageToCH-fromCH {ℓ = ℓ} K CH =
  let module CommonCH = Common {ℓ = ℓ} K CH in
  record
    { S   = StageIndexℕ⁺
    ; SS  = CommonCH.SS
    ; SC  = CommonCH.SC
...
StageToCH-fromCH-μFlow {ℓ = ℓ} {Sig = Sig} {Q = Q} K CH ωCPO FF =
  ...
  where
    module CommonCH = Common {ℓ = ℓ} K CH
