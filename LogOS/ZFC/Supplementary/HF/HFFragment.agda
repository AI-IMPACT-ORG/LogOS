...
{-# OPTIONS --safe #-}
module LogOS.ZFC.Supplementary.HF.HFFragment where
...
infix 4 _∈HF_ _≈HF_ _⊆HF_ ObsEqHF
...
_≈HF_ : HF → HF → Set
-- Extensional equality: mutual refinement in the subset preorder.
x ≈HF y = (x ⊆HF y) × (y ⊆HF x)

-- Presentation alias: pointwise membership equivalence (`↔`).
ObsEqHF : HF → HF → Set
ObsEqHF x y = ∀ z → (z ∈HF x) ↔ (z ∈HF y)

ObsEqHF↔≈HF : ∀ {x y} → ObsEqHF x y ↔ x ≈HF y
ObsEqHF↔≈HF {x} {y} =
  intro
    (λ eq →
      ( (λ z zx → _↔_.to (eq z) zx)
      , (λ z zy → _↔_.from (eq z) zy)
      ))
    (λ (xy , yx) z → intro (xy z) (yx z))
...
refl≈HF  : ∀ x → x ≈HF x
refl≈HF x = (refl⊆HF x , refl⊆HF x)

sym≈HF   : ∀ {x y} → x ≈HF y → y ≈HF x
sym≈HF (xy , yx) = (yx , xy)

trans≈HF : ∀ {x y z} → x ≈HF y → y ≈HF z → x ≈HF z
trans≈HF (xy , yx) (yz , zy) =
  (trans⊆HF xy yz , trans⊆HF zy yx)
...
-- Extensionality axiom: direct from pointwise `↔`.

extensionalityHF : ∀ x y → ObsEqHF x y → x ≈HF y
extensionalityHF x y e = _↔_.to ObsEqHF↔≈HF e

-- Membership respects extensional equality

mem-ext-HF : ∀ {x y} → x ≈HF y → ∀ z → (z ∈HF x) ↔ (z ∈HF y)
mem-ext-HF e z = _↔_.from ObsEqHF↔≈HF e z
