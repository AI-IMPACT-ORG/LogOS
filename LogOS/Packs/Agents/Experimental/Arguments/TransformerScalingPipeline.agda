...
module For
  ...
  record LossDynamics {g : Scale} (s : RGStep g) : Set (lsuc (lsuc ℓ)) where
    field
      ...
      order : TB.LossOrderReflecting (ScalingDimension.obs dim)
  ...
  nextTokenLossDynamics
    ...
    → TB.LossOrderReflecting (TB.NextTokenLossObservableFromData.obs L)
  ...
  record PipelineAssumptions {g : Scale} : Set (lsuc (lsuc ℓ)) where
    field
      ...
      lossOrder
        : TB.LossOrderReflecting
            (TB.LossObservableFromData.obs (TB.TrainingSpec.lossObsData spec))
      lossMonotone
        : ∀ p
        → _≤s_
            (TB.lossParam
              (TB.LossObservableFromData.lossData (TB.TrainingSpec.lossObsData spec))
              (TB.TrainingSpec.trainParam spec p))
            (TB.lossParam
              (TB.LossObservableFromData.lossData (TB.TrainingSpec.lossObsData spec)) p)
  ...
  record Pipeline {g : Scale} : Set (lsuc (lsuc ℓ)) where
    ...
    lossObsData =
      TB.TrainingSpec.lossObsData (TB.OptimizerTraining.spec optimizer)
