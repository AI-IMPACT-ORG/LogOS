...
{-# OPTIONS --safe #-}
module LogOS.Packs.Agents.Experimental.Arguments.ScalingLaws where
...
module For
  ...
  ScalingBound s D γ =
    _≤s_ (ScalingDimension.obs D (rg-μ s))
         (ScalingDimension.obs D (decodeₖ γ))
  ...
  scalingBound-reify
    : ∀ {g} {s : RGStep g} {D : ScalingDimension s} γ
    → ScalingBound s D (reifyₖ γ) ↔ ScalingBound s D γ
  scalingBound-reify {s = s} {D = D} γ =
    let eq = GradedKernel.reify-decode K γ in
    record
      { to   = λ bound → scalingBound-ext {s = s} {D = D} (reifyₖ γ) γ eq bound
      ; from = λ bound → scalingBound-ext {s = s} {D = D} γ (reifyₖ γ) (sym eq) bound
      }
