...
{-# OPTIONS --safe #-}
module LogOS.Packs.Agents.Learning.RGFlow where
...
-- RG-flow stability for learning: graded closure steps as coarse-graining maps,
-- with μ-fixed points and quantale-grade composition.
...
module For
  ...
  open μ using (_⊑_; iter; μF; μF-unfold-left; μF-induction;
                ScottContinuous; iter-mono-chain-infl; μF-unfold-right-infl)
  ...
  rg-iter s = iter (Endo.fn (ClosureStepAt.endo s))

  rg-μ : ∀ {g} → RGStep g → Policy
  rg-μ s = μF (Endo.fn (ClosureStepAt.endo s))
  ...
  rg-unfold-left s =
    μF-unfold-left (Endo.fn (ClosureStepAt.endo s)) (Endo.mono (ClosureStepAt.endo s))
  ...
  rg-induction s c pre =
    μF-induction (Endo.fn (ClosureStepAt.endo s)) (Endo.mono (ClosureStepAt.endo s)) c pre

  rg-chain
    : ∀ {g} (s : RGStep g)
    → ∀ n → _⊑_ (rg-iter s n) (rg-iter s (suc n))
  rg-chain s =
    iter-mono-chain-infl (Endo.fn (ClosureStepAt.endo s)) (ClosureStepAt.infl s)
  ...
  rg-unfold-right s SC =
    μF-unfold-right-infl (Endo.fn (ClosureStepAt.endo s)) SC (ClosureStepAt.infl s)
  ...
  record CFunctionTime (F : RGTimeFlow) : Set (lsuc (lsuc ℓ)) where
    field
      cfun : Policy → Time
      mono : ∀ {c d} → _⊑_ c d → QAdapter._≤s_ Q (τ (cfun c)) (τ (cfun d))
      step : ∀ t c → QAdapter._≤s_ Q
               (τ (cfun (applyRG (RGTimeFlow.step F t) c)))
               (τ (cfun c))

  record AFunctionTime (F : RGTimeFlow) : Set (lsuc (lsuc ℓ)) where
    field
      afun : Policy → Time
      mono : ∀ {c d} → _⊑_ c d → QAdapter._≤s_ Q (τ (afun c)) (τ (afun d))
      step : ∀ t c → QAdapter._≤s_ Q
               (τ (afun (applyRG (RGTimeFlow.step F t) c)))
               (τ (afun c))
  ...
  c-from-flow
    : ∀ {F : RGTimeFlow} → CFunctionTime F → (t : Time) → CFunction (RGTimeFlow.step F t)
  c-from-flow CF t =
    record
  ...
  a-from-flow
    : ∀ {F : RGTimeFlow} → AFunctionTime F → (t : Time) → AFunction (RGTimeFlow.step F t)
  a-from-flow AF t =
    record
  ...
  record ScalingDimensionTime (F : RGTimeFlow) : Set (lsuc (lsuc ℓ)) where
    field
      action : ScaleAction
      obs : Policy → QAdapter.Scale Q
      mono : ∀ {c d} → _⊑_ c d → QAdapter._≤s_ Q (obs c) (obs d)
      scale : ∀ t c → QAdapter._≤s_ Q
        (obs (applyRG (RGTimeFlow.step F t) c))
