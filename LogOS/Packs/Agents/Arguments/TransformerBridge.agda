...
{-# OPTIONS --safe #-}
module LogOS.Packs.Agents.Arguments.TransformerBridge where
...
module For
  ...
  record TrainingEndo (g : Scale) : Set (lsuc (lsuc ℓ)) where
    field
      endo : GEndo.Endo K
      infl : GEndo._≤₂_ K (GEndo.idEndo K) endo
      leFlow : GEndo._≤₂_ K endo (GEndo.Flow-EndoAt K g)

  rgStepFromEndo
    : ∀ {g} → TrainingEndo g → RGStep g
  rgStepFromEndo A =
    GEndo.mkClosureStepAt (TrainingEndo.endo A) (TrainingEndo.infl A) (TrainingEndo.leFlow A)
  ...
  -- Next-token cross-entropy: treat predicted tokens as distributions.
  record CrossEntropyToken (ops : TransformerOps) : Set (lsuc (lsuc ℓ)) where
    field
      prob : TransformerOps.Token ops → TransformerOps.Token ops → TransformerOps.Scalar ops
      negLog : TransformerOps.Scalar ops → Scale
      aggregateTokens : List Scale → Scale

  crossEntropyLoss
    : ∀ {ops}
    → CrossEntropyToken ops
    → List (TransformerOps.Token ops)
    → List (TransformerOps.Token ops)
    → Scale
  crossEntropyLoss {ops} C preds targets =
    let open CrossEntropyToken C in
    aggregateTokens (zipWith (λ p t → negLog (prob p t)) preds targets)

  record NextTokenCrossEntropyData (ops : TransformerOps) : Set (lsuc (lsuc ℓ)) where
    field
      Sample : Set ℓ
      samples : List Sample
      prefix : Sample → List (TransformerOps.Token ops)
      nextTokens : Sample → List (TransformerOps.Token ops)
      ceTok : CrossEntropyToken ops
      aggregateSamples : List Scale → Scale

  nextTokenLossData
    : ∀ {ops} → NextTokenCrossEntropyData ops → LossData ops
  nextTokenLossData {ops} D =
    record
      { Sample = NextTokenCrossEntropyData.Sample D
      ; samples = NextTokenCrossEntropyData.samples D
      ; input = NextTokenCrossEntropyData.prefix D
      ; target = NextTokenCrossEntropyData.nextTokens D
      ; loss = crossEntropyLoss (NextTokenCrossEntropyData.ceTok D)
      ; aggregate = NextTokenCrossEntropyData.aggregateSamples D
      }
  ...
  record LossOrder (obs : Policy → Scale) : Set (lsuc (lsuc ℓ)) where
    field
  ...
  lossDecrease-stable obs O dec c =
    record { closed = LossOrder.reflect O (dec c) }
  ...
  -- Cross-entropy loss made explicit as a LossObservableFromData.
  record NextTokenLossObservableFromData (B : TransformerKernelBridge) : Set (lsuc (lsuc ℓ)) where
    private
      ops = TransformerKernelBridge.ops B
    field
      xentData : NextTokenCrossEntropyData ops
      action : RG.ScaleAction
      obs : Policy → Scale
      obs-encode
        : ∀ (p : TransformerOps.Param ops)
        → obs (TransformerKernelBridge.encode B p)
          ≡ lossParam (nextTokenLossData xentData) p
      mono : ∀ {c d} → _⊑_ c d → QAdapter._≤s_ Q (obs c) (obs d)
      scale : ∀ {g} (s : RGStep g) → ∀ c
            → QAdapter._≤s_ Q
                (obs (applyRG s c))
    ...
    asLossObservableFromData =
      record
        { lossData = nextTokenLossData xentData
        ; action = action
        ; obs = obs
  ...
  record OptimizerTraining (g : Scale) : Set (lsuc (lsuc ℓ)) where
    field
      spec : TrainingSpec g
      lossObsData : LossObservableFromData (TrainingSpec.bridge spec)
      lossOrder : LossOrder (LossObservableFromData.obs lossObsData)
      lossDecrease
        : ∀ p
        → QAdapter._≤s_ Q
            (lossParam (LossObservableFromData.lossData lossObsData)
              (TrainingSpec.trainParam spec p))
            (lossParam (LossObservableFromData.lossData lossObsData) p)

  optimizer-policy-decrease
    : ∀ {g} (O : OptimizerTraining g)
    → ∀ p
    → QAdapter._≤s_ Q
        (LossObservableFromData.obs (OptimizerTraining.lossObsData O)
  ...
  optimizer-param-stable O p =
    record
      { closed = LossOrder.reflect (OptimizerTraining.lossOrder O)
                  (optimizer-policy-decrease O p)
      }
  ...
  trainingBridgeFromSpec S =
    record
      ...
      ; dim = scalingDimensionFromLoss (TrainingSpec.lossObs S)
