...
_≤₂_ : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
       (K : UngradedKernel Sig Q) → Endo K → Endo K → Set ℓ
_≤₂_ {ℓ} {Sig} {Q} K f g = ∀ c →
  let open UngradedKernel K in
...
refl₂ : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
        (K : UngradedKernel Sig Q) (f : Endo K) → _≤₂_ K f f
refl₂ K f = λ _ →
  let open UngradedKernel K in
...
≈₂-sym
  : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
    {K : UngradedKernel Sig Q} {f g : Endo K}
  → _≈₂_ K f g → _≈₂_ K g f
≈₂-sym (fg , gf) = (gf , fg)
