...
{-# OPTIONS --safe #-}
module LogOS.Kernel.LogicKernel.BudgetedTier where
...
record BudgetedTier
  ...
  open GTier (LogicKernel.G K) renaming (Step to Grade)
  ...
  field
    ...
    sat-top  : ∀ g → _≤g_ g (GTier.sat (LogicKernel.G K))
    ...
    mono-grade
      ...
      → _≤g_ g g'
      → ∀ c
      → _⊑_ (GTier.Flow (LogicKernel.G K) g c) (GTier.Flow (LogicKernel.G K) g' c)

    -- Lax step composition matches monoid multiplication.
    comp-lax
      : ∀ g g' c
      → _⊑_
          (GTier.Flow (LogicKernel.G K) g'
            (GTier.Flow (LogicKernel.G K) g c))
          (GTier.Flow (LogicKernel.G K) (_·g_ g g') c)
...
stepOrderOfBudgetedTier {K = K} BT c =
  mono-grade BT (sat-top BT (GTier.step (LogicKernel.G K))) c
...
module Derived
  ...
  boxAt-mono-grade
    ...
    → Core.Code≤ (LogicKernel.shape K) (BoxAt K g γ) (BoxAt K g' γ)
  boxAt-mono-grade {g} {g'} g≤g' γ
    rewrite decode-BoxAt K g γ | decode-BoxAt K g' γ
    = mono-gradeᵇ g≤g' (LogicKernel.decode K γ)

  boxAt-comp-lax
    : ∀ g g' (γ : LogicKernel.Code K)
  ...
  boxAt-comp-lax g g' γ
    ...
    = comp-laxᵇ g g' (LogicKernel.decode K γ)

  boxAt≤Box
    : ∀ g (γ : LogicKernel.Code K)
    → Core.Code≤ (LogicKernel.shape K) (BoxAt K g γ) (Box K γ)
  boxAt≤Box g γ =
...
budgetedTierFromKernel
  : ∀ {ℓ : Level} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
    (K₀ : K.Kernel Sig Q)
  → BudgetedTier (LKFromKernel.asLogicKernel K₀)
budgetedTierFromKernel K₀ =
  let
    K'  = LKFromKernel.asLogicKernel K₀
    CP∂ = BulkBoundary.bnd (LogicKernel.BB K')
  in
  record
    { _≤g_ = λ _ _ → ⊤
    ...
    ; comp-lax = λ _ _ c → GTier.idemp-sat (LogicKernel.G K') c
    }

budgetedTierFromGradedKernel
  : ∀ {ℓ : Level} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
    (K₀ : KG.GradedKernel Sig Q)
  → BudgetedTier (LKFromGraded.asLogicKernel K₀)
