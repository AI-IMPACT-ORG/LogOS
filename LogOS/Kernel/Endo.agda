...
Th⋆K : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
       (K : Kernel Sig Q) → ConPreorder.Con (BulkBoundary.bnd (Kernel.BB K))
Th⋆K K = GTier.Th* (Kernel.G K)
...
Th⋆≤FlowTh⋆ K = GTier.Th*-fixed⇒ (Kernel.G K)

FlowTh⋆≤Th⋆
  : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
    (K : Kernel Sig Q)
  → ConPreorder._⊑_ (BulkBoundary.bnd (Kernel.BB K)) (FlowTh⋆K K) (Th⋆K K)
FlowTh⋆≤Th⋆ K = GTier.Th*-fixed⇐ (Kernel.G K)
