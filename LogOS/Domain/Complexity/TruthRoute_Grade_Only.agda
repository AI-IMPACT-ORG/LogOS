...
open import LogOS.Prelude.Product using (Σ; _,_; proj₁; proj₂; _×_; snd)
...
module NonUniformNat
  ...
  PolyTotalWitnessedVerification : ∀ {ℓA} (Acc : Con → Set ℓA) → Set (ℓ ⊔ ℓI ⊔ ℓP ⊔ ℓA)
  ...
  module WithWitnessSize (WSize₀ : GradedKernel.Code K → ℕ) where
    ...
    PolyTotalWitnessedVerificationW : ∀ {ℓA} (Acc : Con → Set ℓA) → Set (ℓ ⊔ ℓI ⊔ ℓP ⊔ ℓA)
