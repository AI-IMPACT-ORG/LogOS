...
{-# OPTIONS --safe #-}
module LogOS.Domain.Complexity.ClassicalPvsNP where
  ...
  module ForLanguage {ℓL : Level} (L : R.Language ℓL) where
    ...
    fromInP P =
      let open R.InP P
          ...
          decide x = Acc (R.Flow (gradeBound (tBound (Size x))) (R.decodeK (DetRun x)))
          D : LW.DeciderI Input L
          D = record
            { decide = decide
            ; total  = λ x → decAcc (R.Flow (gradeBound (tBound (Size x))) (R.decodeK (DetRun x)))
      ...
      where
        ...
        WS = record
          ...
          where
            ...
            Check x (w , _) =
              Acc (R.Flow (gradeBound (tBound (Size x))) (R.decodeK (VerRunWith x w)))

            decCheck : ∀ x w → Check x w ⊎ ¬ Check x w
            decCheck x (w , _) =
              decAcc (R.Flow (gradeBound (tBound (Size x))) (R.decodeK (VerRunWith x w)))
