...
{-# OPTIONS --safe #-}
module LogOS.InfoTheory.Shannon.Core where
...
module For (F : ShannonFacts) where
  ...
  -- Finite KL divergence (as a total sum, relying on `klTerm`’s explicit zero law).
  ...
  -- Shannon entropy (as “negative self-KL term”, i.e. -Σ p ln p with explicit zero law).
  H : ∀ {n : ℕ} → Dist n → ℝ
