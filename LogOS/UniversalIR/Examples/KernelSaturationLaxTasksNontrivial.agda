...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Examples.KernelSaturationLaxTasksNontrivial where
...
open import LogOS.API.Kernel using (Kernel; GTier)
...
GTierℕ =
  record
    ...
    ; sat       = tt
    ; Flow      = λ _ c → Flowℕ c
    ; mono      = λ {g} le → Flowℕ-mono le
    ; infl-sat  = Flowℕ-infl
    ; idemp-sat = Flowℕ-idemp≤
    ; Th*       = Thℕ
...
module K = ForKernel Kℕ (record { mono-Body∂ = λ le → le })
