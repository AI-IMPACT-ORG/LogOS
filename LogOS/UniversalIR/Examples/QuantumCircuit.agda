...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Examples.QuantumCircuit where
...
open import LogOS.UniversalIR.Task using (PATask; mkTask; Add)
open import LogOS.UniversalIR.Languages.QuantumCircuit as QC using (compilePrepBrand; fuelPrep)
import LogOS.Computation.Scheme as Sch
import LogOS.Computation.SchemeCategory as Cat
open import LogOS.UniversalIR.Schemes using (QuantumCircuitProcess; meas; meas-zero; meas-+; budget₂; meas≤budget₂; QSteps)
...
qcInterface : Cat.Interface QuantumCircuitCode QuantumCircuitProcess
qcInterface = record { compile = (λ q → q) ; fuel = (λ _ → 1) }

qcScheme : Sch.Scheme QuantumCircuitCode ℕ
qcScheme = Cat.schemeFromInterface QuantumCircuitProcess qcInterface
...
-- Using `meas-+`: sequential measurements add along the second prequantale axis.

qcInterface2 : Cat.Interface QuantumCircuitCode QuantumCircuitProcess
qcInterface2 = record { compile = (λ q → q) ; fuel = (λ _ → 2) }

qcScheme2 : Sch.Scheme QuantumCircuitCode ℕ
qcScheme2 = Cat.schemeFromInterface QuantumCircuitProcess qcInterface2
