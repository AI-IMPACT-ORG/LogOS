...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Examples.QuantumCircuitAmp where
...
open import LogOS.UniversalIR.Core.QuantumCircuitAmp as QCA
...
measureBranches-def :
  measureBranches ≡
    record
      { support =
          (probFalse fzero (QuantumCircuitAmpPCode.state qAfterH)
          , setPCQCAP 2 (setStateQCAP (collapseFalseRaw fzero (QuantumCircuitAmpPCode.state qAfterH)) qAfterH))
          ∷ (probTrue fzero (QuantumCircuitAmpPCode.state qAfterH)
          , setPCQCAP 2 (setStateQCAP (collapseTrueRaw fzero (QuantumCircuitAmpPCode.state qAfterH)) qAfterH))
          ∷ []
      }
