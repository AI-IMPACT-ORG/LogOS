...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Theorems where
...
open import LogOS.UniversalIR.IR using (lowerToIR; decode; observe; take; bitsToNat; double)
open import LogOS.UniversalIR.Std
...
open import LogOS.UniversalIR.Languages.QuantumOracle as Oracle
open import LogOS.UniversalIR.Languages.QuantumCircuit as Circuit
open import LogOS.UniversalIR.CQM.QuantumCircuitRel public
open import LogOS.UniversalIR.Encoding as Enc using (incBits; natToBits; length; take-length; bitsToNat-natToBits)
open import LogOS.UniversalIR.Universality using (simulateUM; simulateUL; simulateUE; simulateUQ; simulateUQC)
...
-- --------------------------------------------------------------------------
-- Shared lemmas are provided by `LogOS.UniversalIR.Std`.
...
-- --------------------------------------------------------------------------
-- Scheme interfaces (same universal carrier `UCode`, different compilation+fuel)
--
...
minskyInterfaceScheme≡run : (t : PATask) → Sch.run minskyScheme t ≡ Minsky.run t
minskyInterfaceScheme≡run t =
  trans
    (cong observe (exec≡simulate (Minsky.fuel t) (Minsky.compile t)))
    refl

lambdaInterfaceScheme≡run : (t : PATask) → Sch.run lambdaScheme t ≡ Lambda.run t
lambdaInterfaceScheme≡run t =
  trans
    (cong observe (exec≡simulate (Lambda.fuel t) (Lambda.compile t)))
    refl

ethereumInterfaceScheme≡run : (t : PATask) → Sch.run ethereumScheme t ≡ Ether.run t
ethereumInterfaceScheme≡run t =
  trans
    (cong observe (exec≡simulate (Ether.fuel t) (Ether.compile t)))
    refl

oracleInterfaceScheme≡run : (t : PATask) → Sch.run oracleScheme t ≡ Oracle.run t
oracleInterfaceScheme≡run t =
  trans
    (cong observe (exec≡simulate (Oracle.fuel t) (Oracle.compile t)))
    refl

circuitInterfaceScheme≡run : (t : PATask) → Sch.run quantumCircuitScheme t ≡ Circuit.run t
circuitInterfaceScheme≡run t =
  trans
...
record InterfaceSchemesCorrect (t : PATask) : Set where
  field
...
record InterfaceSchemesCostBound (t : PATask) : Set where
  field
...
patask-interfaceSchemes-correct : ∀ t → InterfaceSchemesCorrect t
patask-interfaceSchemes-correct t =
  record
    { minsky   = trans (minskyInterfaceScheme≡run t) (minsky-correct t)
    ; lambda   = trans (lambdaInterfaceScheme≡run t) (lambda-correct t)
    ; ethereum = trans (ethereumInterfaceScheme≡run t) (ethereum-correct t)
    ; oracle   = trans (oracleInterfaceScheme≡run t) (oracle-correct t)
    ; circuit  = trans (circuitInterfaceScheme≡run t) (circuit-correct t)
    }
...
patask-interfaceSchemes-costBound : ∀ t → InterfaceSchemesCostBound t
patask-interfaceSchemes-costBound t =
  record
    { minsky   = interfaceScheme-cost≤budget₂3n,n minskyInterface t
    ; lambda   = interfaceScheme-cost≤budget₂3n,n lambdaInterface t
    ; ethereum = interfaceScheme-cost≤budget₂3n,n ethereumInterface t
    ; oracle   = interfaceScheme-cost≤budget₂3n,n oracleInterface t
    ; circuit  = interfaceScheme-cost≤budget₂3n,n quantumCircuitInterface t
    }
...
minskyScheme-runComputesWithinFuelBudgetFor t =
  FS-minsky.runIsComputesWithin minskyScheme-fuelHalts t (fuelBudgetFor minskyScheme t)
    (InterfaceSchemesCostBound.minsky (patask-interfaceSchemes-costBound t))
...
lambdaScheme-runComputesWithinFuelBudgetFor t =
  FS-lambda.runIsComputesWithin lambdaScheme-fuelHalts t (fuelBudgetFor lambdaScheme t)
    (InterfaceSchemesCostBound.lambda (patask-interfaceSchemes-costBound t))
...
ethereumScheme-runComputesWithinFuelBudgetFor t =
  FS-ethereum.runIsComputesWithin ethereumScheme-fuelHalts t (fuelBudgetFor ethereumScheme t)
    (InterfaceSchemesCostBound.ethereum (patask-interfaceSchemes-costBound t))
...
oracleScheme-runComputesWithinFuelBudgetFor t =
  FS-oracle.runIsComputesWithin oracleScheme-fuelHalts t (fuelBudgetFor oracleScheme t)
    (InterfaceSchemesCostBound.oracle (patask-interfaceSchemes-costBound t))
...
quantumCircuitScheme-runComputesWithinFuelBudgetFor t =
  FS-circuit.runIsComputesWithin quantumCircuitScheme-fuelHalts t (fuelBudgetFor quantumCircuitScheme t)
    (InterfaceSchemesCostBound.circuit (patask-interfaceSchemes-costBound t))

patask-interfaceSchemes-runEq : InterfaceSchemesRunEq
patask-interfaceSchemes-runEq =
  record
    { minsky≈lambda = λ t →
        let c = patask-interfaceSchemes-correct t in
        trans (InterfaceSchemesCorrect.minsky c)
              (sym (InterfaceSchemesCorrect.lambda c))
    ; lambda≈ethereum = λ t →
        let c = patask-interfaceSchemes-correct t in
        trans (InterfaceSchemesCorrect.lambda c)
              (sym (InterfaceSchemesCorrect.ethereum c))
    ; ethereum≈oracle = λ t →
        let c = patask-interfaceSchemes-correct t in
        trans (InterfaceSchemesCorrect.ethereum c)
              (sym (InterfaceSchemesCorrect.oracle c))
    ; oracle≈circuit = λ t →
        let c = patask-interfaceSchemes-correct t in
        trans (InterfaceSchemesCorrect.oracle c)
              (sym (InterfaceSchemesCorrect.circuit c))
    }
...
minsky-implements-PA = record { correct = λ t → InterfaceSchemesCorrect.minsky (patask-interfaceSchemes-correct t) }

lambda-implements-PA : Sch.ImplementsRun PAAlg lambdaScheme
lambda-implements-PA = record { correct = λ t → InterfaceSchemesCorrect.lambda (patask-interfaceSchemes-correct t) }

ethereum-implements-PA : Sch.ImplementsRun PAAlg ethereumScheme
ethereum-implements-PA = record { correct = λ t → InterfaceSchemesCorrect.ethereum (patask-interfaceSchemes-correct t) }

oracle-implements-PA : Sch.ImplementsRun PAAlg oracleScheme
oracle-implements-PA = record { correct = λ t → InterfaceSchemesCorrect.oracle (patask-interfaceSchemes-correct t) }

circuit-implements-PA : Sch.ImplementsRun PAAlg quantumCircuitScheme
circuit-implements-PA = record { correct = λ t → InterfaceSchemesCorrect.circuit (patask-interfaceSchemes-correct t) }
...
module Q = AppKit.MakeDerived Assumptions Claim
  (λ _ →
    record
      ...
      ; minskyCost     = λ t → InterfaceSchemesCostBound.minsky (patask-interfaceSchemes-costBound t)
      ; lambdaCost     = λ t → InterfaceSchemesCostBound.lambda (patask-interfaceSchemes-costBound t)
      ; ethereumCost   = λ t → InterfaceSchemesCostBound.ethereum (patask-interfaceSchemes-costBound t)
      ; oracleCost     = λ t → InterfaceSchemesCostBound.oracle (patask-interfaceSchemes-costBound t)
      ; circuitCost    = λ t → InterfaceSchemesCostBound.circuit (patask-interfaceSchemes-costBound t)
      ; minsky≈lambda   = InterfaceSchemesRunEq.minsky≈lambda patask-interfaceSchemes-runEq
      ; lambda≈ethereum = InterfaceSchemesRunEq.lambda≈ethereum patask-interfaceSchemes-runEq
      ; ethereum≈oracle = InterfaceSchemesRunEq.ethereum≈oracle patask-interfaceSchemes-runEq
      ; oracle≈circuit  = InterfaceSchemesRunEq.oracle≈circuit patask-interfaceSchemes-runEq
      })
