...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Schemes where

open import LogOS.Prelude
open import LogOS.Syntax.Prop using (¬_)
open import LogOS.Prelude using (ℕ; suc; zero; _+_)
...
open import LogOS.Minimal.Con using (ConPreorder; EqConPreorder)
...
import LogOS.UniversalIR.Core.QuantumCircuitAmp

open import LogOS.UniversalIR.Core.Utils using (BoundaryObs)
import LogOS.UniversalIR.ObservedKernel as ObsKernel
...
open import LogOS.UniversalIR.IR using (lowerToIR; observe)
...
open import LogOS.UniversalIR.Languages.Ethereum as Ethereum
open import LogOS.UniversalIR.Languages.QuantumOracle as QuantumOracle
open import LogOS.UniversalIR.Languages.QuantumCircuit as QuantumCircuit
open import LogOS.Computation.Core using (iterate)

-- The canonical cost adapter for the UniversalIR scheme layer.
--
-- `QNat2` is a two-axis (finite-join) prequantale on `ℕ × ℕ`:
...
-- Cost helpers (two-axis prequantale split)
--
...
-- Measurement-axis algebra (the second axis is additive under prequantale
...
--
-- This makes the prequantale structure operational in the universality story:
...
-- A non-trivial prequantale-cost profile (e.g. “gas”) for the universal carrier.
-- These weights are intentionally simple and purely structural: they show how
-- the prequantale algebra composes costs across heterogeneous machine steps.
...
--
-- This is where the finite-join prequantale structure starts to pay off: the
...
-- Scheme-level cost envelope for any interface into the universal process.
--
...
costExecP≡costExecᵁ
  : ∀ {ℓI} {Input : Set ℓI}
    (I : Cat.Interface Input UProcess)
  → ∀ n u
  → Cat.costExecP UProcess n u ≡ Sch.Scheme.costExec (Cat.schemeFromInterface UProcess I) n u
costExecP≡costExecᵁ _ zero    _ = refl
costExecP≡costExecᵁ I (suc n) u =
  cong (stepCostᵁ u ·_) (costExecP≡costExecᵁ I n (stepU u))

costExecᵁ≤budget₂3n,n
  : ∀ {ℓI} {Input : Set ℓI}
    (I : Cat.Interface Input UProcess)
  → ∀ n u
  → QAdapter._≤s_ QSteps (Sch.Scheme.costExec (Cat.schemeFromInterface UProcess I) n u) (budget₂ (three· n) n)
costExecᵁ≤budget₂3n,n I n u =
  subst
    (λ c → QAdapter._≤s_ QSteps c (budget₂ (three· n) n))
    (costExecP≡costExecᵁ I n u)
    (costExecPᵁ≤budget₂3n,n n u)

interfaceScheme-cost≤budget₂3n,n
  : ∀ {ℓI} {Input : Set ℓI}
    (I : Cat.Interface Input UProcess)
  → ∀ x
  → QAdapter._≤s_ QSteps (Sch.cost (Cat.schemeFromInterface UProcess I) x)
      (budget₂ (three· (Cat.Interface.fuel I x)) (Cat.Interface.fuel I x))
interfaceScheme-cost≤budget₂3n,n I x =
  costExecᵁ≤budget₂3n,n I (Cat.Interface.fuel I x) (Cat.Interface.compile I x)

-- ============================================================================
-- Budgeted execution (prequantale-graded, compositional)
--
-- `interfaceScheme-cost≤budget₂3n,n` bounds the *total* fuel-bounded run cost.
-- With `QAdapter.·-mono` available, we can also expose a more operational view:
-- budgeted executions compose under prequantale multiplication.
-- ============================================================================
...
interfaceScheme-execWithinAt≤budget₂3n,n
  ...
  → ∀ x
  → Sch.ExecWithin
      (Cat.schemeFromInterface UProcess I)
      (Cat.Interface.fuel I x)
      (Cat.Interface.compile I x)
      (budget₂ (three· (Cat.Interface.fuel I x)) (Cat.Interface.fuel I x))
      (Sch.exec (Cat.schemeFromInterface UProcess I) (Cat.Interface.fuel I x) x)
interfaceScheme-execWithinAt≤budget₂3n,n I x =
  LogOS.Prelude.refl ,
  costExecᵁ≤budget₂3n,n I (Cat.Interface.fuel I x) (Cat.Interface.compile I x)
...
interfaceScheme-execWithinSplit≤budget₂3n,n
  ...
  → Sch.ExecWithin
      ...
      (iterate (Sch.Scheme.Comp (Cat.schemeFromInterface UProcess I)) n
        (iterate (Sch.Scheme.Comp (Cat.schemeFromInterface UProcess I)) m u))
interfaceScheme-execWithinSplit≤budget₂3n,n I m n u =
  Sch.ExecWithin-+
    S
    {m = m}
    {n = n}
...
mkInterface
  : ∀ {ℓI ℓO ℓC ℓQ}
    ...
    → (Input → ℕ)
    → Cat.Interface Input P
mkInterface _ compile fuel = record { compile = compile ; fuel = fuel }
...
mkScheme
  : ∀ {ℓI ℓO ℓC ℓQ}
    ...
    → (Input → ℕ)
    → Scheme Input Output
mkScheme P compile fuel = Cat.schemeFromInterface P (mkInterface P compile fuel)

minskyInterface : Cat.Interface PATask UProcess
minskyInterface = mkInterface UProcess Minsky.compile Minsky.fuel

lambdaInterface : Cat.Interface PATask UProcess
lambdaInterface = mkInterface UProcess Lambda.compile Lambda.fuel

ethereumInterface : Cat.Interface PATask UProcess
ethereumInterface = mkInterface UProcess Ethereum.compile Ethereum.fuel

oracleInterface : Cat.Interface PATask UProcess
oracleInterface = mkInterface UProcess QuantumOracle.compile QuantumOracle.fuel

quantumCircuitInterface : Cat.Interface PATask UProcess
quantumCircuitInterface = mkInterface UProcess QuantumCircuit.compile QuantumCircuit.fuel

-- Five concrete “representation schemes” for the same PATask meaning.
-- (Same process, different interfaces.)
...
run≤-fuel≡run-interface
  : ∀ (I : Cat.Interface PATask UProcess) (t : PATask)
  → Sch.run≤ (Cat.schemeFromInterface UProcess I) OpsSteps (fuelGrade (Cat.Interface.fuel I) t) t
    ≡ Sch.run (Cat.schemeFromInterface UProcess I) t
run≤-fuel≡run-interface I t =
  cong
    (λ k → Sch.decode S (Sch.close S (Sch.exec S k t)))
    (sym (Q2.steps-budget-τ (Cat.Interface.fuel I t)))
  where
    S = Cat.schemeFromInterface UProcess I

run≤-fuel≡run-minsky : ∀ t → run≤-minsky (fuelGrade Minsky.fuel t) t ≡ Sch.run minskyScheme t
run≤-fuel≡run-minsky t = run≤-fuel≡run-interface minskyInterface t

run≤-fuel≡run-lambda : ∀ t → run≤-lambda (fuelGrade Lambda.fuel t) t ≡ Sch.run lambdaScheme t
run≤-fuel≡run-lambda t = run≤-fuel≡run-interface lambdaInterface t

run≤-fuel≡run-ethereum : ∀ t → run≤-ethereum (fuelGrade Ethereum.fuel t) t ≡ Sch.run ethereumScheme t
run≤-fuel≡run-ethereum t = run≤-fuel≡run-interface ethereumInterface t

run≤-fuel≡run-oracle : ∀ t → run≤-oracle (fuelGrade QuantumOracle.fuel t) t ≡ Sch.run oracleScheme t
run≤-fuel≡run-oracle t = run≤-fuel≡run-interface oracleInterface t

run≤-fuel≡run-quantumCircuit
  : ∀ t → run≤-quantumCircuit (fuelGrade QuantumCircuit.fuel t) t ≡ Sch.run quantumCircuitScheme t
run≤-fuel≡run-quantumCircuit t = run≤-fuel≡run-interface quantumCircuitInterface t
...
module QuantumCircuitAmpSchemes {ℓ} (S : QScalars {ℓ}) where
  module A = LogOS.UniversalIR.Core.QuantumCircuitAmp.For S
...
module QuantumCircuitAmpFree where
  open import LogOS.UniversalIR.Quantum.Scalars.Free using (formalScalars)
  open QuantumCircuitAmpSchemes formalScalars public

minskyMachineInterface : Cat.Interface PATask MinskyProcess
minskyMachineInterface = mkInterface MinskyProcess Minsky.compileBrand Minsky.fuel

ethereumMachineInterface : Cat.Interface PATask EthereumProcess
ethereumMachineInterface = mkInterface EthereumProcess Ethereum.compileBrand Ethereum.fuel

lambdaMachineInterface : Cat.Interface PATask LambdaProcess
lambdaMachineInterface = mkInterface LambdaProcess Lambda.compileBrand Lambda.fuel

oracleMachineInterface : Cat.Interface PATask QuantumOracleProcess
oracleMachineInterface = mkInterface QuantumOracleProcess QuantumOracle.compileBrand QuantumOracle.fuel

quantumCircuitMachineInterface : Cat.Interface PATask QuantumCircuitProcess
quantumCircuitMachineInterface =
  mkInterface QuantumCircuitProcess QuantumCircuit.compileBrand QuantumCircuit.fuel
...
quantumCircuitFamilyInterface : Cat.Interface (Bounded PATask) QuantumCircuitProcess
quantumCircuitFamilyInterface =
  mkInterface QuantumCircuitProcess
...
quantumCircuitFamilyScheme =
  Cat.schemeFromInterface QuantumCircuitProcess quantumCircuitFamilyInterface
...
Minsky→UCostAt obs =
  record
    ...
    ; stepCost≤ = λ _ → QAdapter.≤s-refl QSteps
    }
