...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Core.QuantumOracle where

open import LogOS.Prelude
open import LogOS.UniversalIR.Core.Minsky using (Reg; R0; R1; R2; R3)
...
-- Program/state split:
-- - program = control instructions (`QInstr` list)
-- - state   = registers + oracle tape + pc

record QuantumProg : Set where
  constructor mkProg
  field
    prog : List QInstr
...
record QuantumCode : Set where
  ...
  open QuantumState State public using (pc; r0; r1; r2; r3; oracle)
  open QuantumProg Prog public using (prog)

pattern mkQ pc r0 r1 r2 r3 oracle prog = mkCode (mkState pc r0 r1 r2 r3 oracle) (mkProg prog)

open QuantumCode public using (pc; r0; r1; r2; r3; oracle; prog)
