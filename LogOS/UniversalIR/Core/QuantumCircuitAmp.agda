...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Core.QuantumCircuitAmp where
...
open import LogOS.UniversalIR.Core.Utils using (BoundaryObs; EffectAt; _⊨ᵇ_; lookupDefault)
open import LogOS.UniversalIR.Quantum.Measurement as QM
...
module For {ℓ} (S : QScalars {ℓ}) where
  ...
  -- - state   = pc + amplitude state

  record QuantumCircuitAmpProg : Set (lsuc ℓ) where
    constructor mkProg
    field
      outLen : ℕ
      prog   : List (QCInstrA outLen)

  record QuantumCircuitAmpState (outLen : ℕ) : Set (lsuc ℓ) where
    constructor mkState
    field
      pc    : ℕ
      amp : State outLen
  ...
  record QuantumCircuitAmpCode : Set (lsuc ℓ) where
    constructor mkCode
    field
      Prog : QuantumCircuitAmpProg
      St   : QuantumCircuitAmpState (QuantumCircuitAmpProg.outLen Prog)

    open QuantumCircuitAmpProg Prog public using (outLen; prog)
    open QuantumCircuitAmpState St public using (pc) renaming (amp to state)

  pattern mkQCA pc outLen state prog =
    mkCode (mkProg outLen prog) (mkState pc state)

  open QuantumCircuitAmpCode public using (pc; outLen; state; prog)
  ...
  record QuantumCircuitAmpPProg (n : ℕ) : Set (lsuc ℓ) where
    constructor mkProgP
    field
      prog : List (QCInstrP n)

  record QuantumCircuitAmpPState (n : ℕ) : Set (lsuc ℓ) where
    constructor mkStateP
    field
      pc    : ℕ
      amp : State n
  ...
  record QuantumCircuitAmpPCode (n : ℕ) : Set (lsuc ℓ) where
    constructor mkCodeP
    field
      Prog : QuantumCircuitAmpPProg n
      St   : QuantumCircuitAmpPState n

    open QuantumCircuitAmpPProg Prog public using (prog)
    open QuantumCircuitAmpPState St public using (pc) renaming (amp to state)

  pattern mkQCAP pc state prog =
    mkCodeP (mkProgP prog) (mkStateP pc state)

  open QuantumCircuitAmpPCode public using (pc; state; prog)
