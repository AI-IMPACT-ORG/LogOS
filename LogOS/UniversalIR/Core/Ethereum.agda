...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.Core.Ethereum where
...
-- - state   = pc/stack/memory

record EVMProg : Set where
  constructor mkProg
  field
    code : List EInstr

record EVMState : Set where
  constructor mkState
  field
    pc    : ℕ
    stack : List ℕ
...
record EVMCode : Set where
  ...
  open EVMState State public using (pc; stack; mem)
  open EVMProg Prog public using (code)

pattern mkE pc stack mem code = mkCode (mkState pc stack mem) (mkProg code)

open EVMCode public using (pc; stack; mem; code)
