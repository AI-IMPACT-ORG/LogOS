...
{-# OPTIONS --safe #-}
module LogOS.UniversalIR.ObservedKernel where

open import LogOS.Prelude

open import LogOS.UniversalIR.Core using (UCode; UM; UL; UE; UQ; UQC; stepU)
open import LogOS.UniversalIR.Core.Lambda using (LambdaCode; mkL; stepLC; var)
open import LogOS.UniversalIR.Core.Minsky using (MinskyCode; mkM; stepM)
open import LogOS.UniversalIR.Core.Ethereum using (EVMCode; mkE; mem0; stepE)
open import LogOS.UniversalIR.Core.QuantumOracle using (QuantumCode; mkQ; stepQ)
open import LogOS.UniversalIR.Core.QuantumCircuit using (QuantumCircuitCode; mkQC; stepQC)
...
open import LogOS.Prelude using (zero)
...
record ObsKit : Set (lsuc lzero) where
  ...
  -- Canonical form: `obsI` is a fixed point up to mutual refinement.
  obsI≈obsStep-obsI : _≈CP_ CPObs obsI (obsStep obsI)
  obsI≈obsStep-obsI = obsI-fixed
...
LambdaObsKit =
  ...
  record
    { base = record
        { CPObs = EqConPreorder LambdaCode
        ; observeU = λ where
            (UL l)  → l
            (UM _)  → l0
            (UE _)  → l0
...
MinskyObsKit =
  ...
  record
    { base = record
        { CPObs = EqConPreorder MinskyCode
        ; observeU = λ where
            (UM m)  → m
            (UL _)  → m0
            (UE _)  → m0
...
QuantumOracleObsKit =
  ...
  record
    { base = record
        { CPObs = EqConPreorder QuantumCode
        ; observeU = λ where
            (UQ q)  → q
            (UM _)  → q0
            (UL _)  → q0
...
QuantumCircuitObsKit =
  ...
  record
    { base = record
        { CPObs = EqConPreorder QuantumCircuitCode
        ; observeU = λ where
            (UQC q) → q
            (UM _)  → qc0
            (UL _)  → qc0
...
module ForObsKit (K : ObsKit) where
  ...
  private
    γ*-guard-obs
      : _≈CP_ CPObs (observeU (encodeObs obsI))
  ...
  GTierObs =
    record
      ...
      ; Flow      = λ _ c → c
      ; mono      = λ le → le
      ; infl-sat  = λ _ → CP.refl
      ; idemp-sat = λ _ → CP.refl
      ; Th*       = obsI
  ...
  module Process where
    module KP = KUP.ForKernel ObsKernel (QAdapter.e Q)
    open KP public
