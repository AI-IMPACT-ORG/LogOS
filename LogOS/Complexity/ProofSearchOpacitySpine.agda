...
{-# OPTIONS --safe #-}
module LogOS.Complexity.ProofSearchOpacitySpine where
...
open import LogOS.Theorems.Meta.Guards using (NonEmptyPred)

import LogOS.Complexity.ProofSearchBoundary as PBₜ
...
module For {ℓ ℓP : Level}
  ...
  record VacuityGuards
    ...
    field
      someWithin : NonEmptyPred Code (B.WithinBudgetBy Bnd)
  ...
  derive A =
    let
      ...
      nontriv : NonTrivialWithinBudget
                  ...
                  (Assumptions.cost A)
                  (Assumptions.budget A)
      nontriv = NonEmptyPred.witness (VacuityGuards.someWithin (Assumptions.vacuity A)) , witness
    in
    record
      { no-total  = noTotal
      ; witness   = witness
