...
{-# OPTIONS --safe #-}
module LogOS.Complexity.PvsNPLedger where
...
open import LogOS.Prelude using (Σ; _,_; proj₁)
...
import LogOS.Complexity.LanguageWitness as LW
import LogOS.Complexity.LanguageWitnessW as LWW
import LogOS.Complexity.TruthRoute_Grade_Only as TR
...
module For {ℓI ℓW ℓ : Level}
  ...
  -- -----------------------------------------------------------------------
  -- Literature wrappers: P = NP and P ≠ NP (language-relative, Input-fixed)
  -- -----------------------------------------------------------------------

  -- “P = NP” for this fixed input domain: every NP language is in P.
  P=NP : Set _
  P=NP = ∀ (L : Language) → InNP L → InP L

  -- “P ≠ NP” for this fixed input domain: there exists a language in NP not in P.
  P≠NP : Set _
  P≠NP = Σ Language (λ L → InNP L × ¬ InP L)

  notInPFromSeparation : ∀ {L} → SeparationW L → ¬ InP L
  notInPFromSeparation sep = notInP (SeparationW.detSuperPoly sep)

  sep→P≠NP : ∀ {L} → SeparationW L → P≠NP
  sep→P≠NP {L = L} sep =
    L , (SeparationW.inNPw sep , notInPFromSeparation sep)
