...
{-# OPTIONS --safe #-}
module LogOS.Complexity.MeasurementCapacity where
...
-- “Inspect” a term: expose it as a value together with its judgmental equality.
--
-- This is useful when a `with`-split needs to transport the knowledge “x is zero/suc”
-- into a proof about the original term `x`.
...
record MeasurementCapacity {ℓ : Level}
  ...
  -- Derived: extracting any nonzero classical information forces a nonzero
  -- measurement count (independent of κ, using only `meas0→info0`).
  info-suc→meas≢0 : ∀ f {k} → info f ≡ suc k → meas f ≡ 0 → ⊥ {lzero}
  info-suc→meas≢0 f {k} infoSuc meas0 =
    suc≢zero (trans (sym infoSuc) (meas0→info0 f meas0))
...
-- Guards are intentionally redundant: an `infoWitness` already forces a
-- nonzero measurement count by `meas0→info0`.
measWitness-from-infoWitness
  : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
    {cap : MeasurementCapacity Sig Q}
  → MeasurementCapacityGuards Sig Q cap
  → Σ (LogOSSignature.Cosp Sig)
      (λ f → Σ ℕ (λ k → MeasurementCapacity.meas cap f ≡ suc k))
measWitness-from-infoWitness {cap = cap} G
  with MeasurementCapacityGuards.infoWitness G
... | f , k , infoSuc
  with inspect (MeasurementCapacity.meas cap f)
... | zero , meas0 =
  ⊥-elim (MeasurementCapacity.info-suc→meas≢0 cap f infoSuc meas0)
... | suc k' , measSuc =
  f , (k' , measSuc)
...
record NonUnitaryCapacity {ℓ : Level}
  ...
  -- Derived: extracting any nonzero classical information forces a nonzero
  -- non-unitary event count (independent of κ, using only `nu0→info0`).
  info-suc→nu≢0 : ∀ f {k} → info f ≡ suc k → nuEvents f ≡ 0 → ⊥ {lzero}
  info-suc→nu≢0 f {k} infoSuc nu0 =
    suc≢zero (trans (sym infoSuc) (nu0→info0 f nu0))
...
-- Guards are intentionally redundant: an `infoWitness` already forces a
-- nonzero non-unitary event count by `nu0→info0`.
nuWitness-from-infoWitness
  : ∀ {ℓ} {Sig : LogOSSignature ℓ} {Q : QAdapter ℓ}
    {cap : NonUnitaryCapacity Sig Q}
  → NonUnitaryCapacityGuards Sig Q cap
  → Σ (LogOSSignature.Cosp Sig)
      (λ f → Σ ℕ (λ k → NonUnitaryCapacity.nuEvents cap f ≡ suc k))
nuWitness-from-infoWitness {cap = cap} G
  with NonUnitaryCapacityGuards.infoWitness G
... | f , k , infoSuc
  with inspect (NonUnitaryCapacity.nuEvents cap f)
... | zero , nu0 =
  ⊥-elim (NonUnitaryCapacity.info-suc→nu≢0 cap f infoSuc nu0)
... | suc k' , nuSuc =
  f , (k' , nuSuc)
