...
{-# OPTIONS --safe #-}
module LogOS.Complexity.UniversalIRCM where
...
import LogOS.Complexity.TruthRoute_Grade_Only as TruthRoute

open import LogOS.UniversalIR.Task as Task using (PATask)
open import LogOS.UniversalIR.Core using (UCode)
open import LogOS.UniversalIR.Blum as UBlum using (BlumU)
open import LogOS.UniversalIR.Schemes as UIS using
  ( UProcess
  ; minskyScheme
  ; lambdaScheme
  ; ethereumScheme
...
import LogOS.UniversalIR.Pack as UIPack
import LogOS.UniversalIR.Size as USize
...
interface : Brand → Cat.Interface PATask UProcess
interface minsky = UIS.minskyInterface
interface lambda = UIS.lambdaInterface
interface ethereum = UIS.ethereumInterface
interface oracle = UIS.oracleInterface
interface quantumCircuit = UIS.quantumCircuitInterface

compile : Brand → PATask → UCode
compile b = Cat.Interface.compile (interface b)

fuel : Brand → PATask → ℕ
fuel b = Cat.Interface.fuel (interface b)
