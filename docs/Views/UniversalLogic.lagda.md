...
import LogOS.Ports.Semantic.SystemIO as SystemIO
...
import LogOS.Ports.Semantic.SatMor as SatMor
...
module Quotes where
  private
    ...
    systemIO-exists : _
    systemIO-exists = SystemIO.SystemIO

    rebase-exists : _
    rebase-exists = SystemIO.rebase

    rebaseAlongSatMor-exists : _
    rebaseAlongSatMor-exists = SystemIO.rebaseAlongSatMor
...
- **Structurality / signature change:** universal logic often builds logics over
  ...
  morphisms (`SigHom`) and reindexing (`reindexKernel`, `reindexLogicKernel`) so
...
- Port/tooling part: stated for a generic satisfaction relation via `PresentationC` and `SystemIO`
...
| Signature morphism / substitution discipline | `SigHom`, `reindexKernel`, `reindexLogicKernel` | Signature change is explicit and transport-first (no hidden substitution principle). |
...
| Logic system with tools | `SystemIO` | Presentation + prover + model-checker; transports are structural. |
| Transport of tools across presentations | `SystemIO.rebase` | Pulls provers/model-checkers back along canonical translations. |
| Morphism between logics | `SatMor`, `SystemIO.rebaseAlongSatMor` | Satisfaction morphism + canonical pullback of tools; no surjectivity assumptions. |
...
`SystemIO`) so that meaning can be held fixed while presentations vary.
...
**Definition (Tool I/O).** A logic system with tooling is a `SystemIO`:
...
\[
  \mathrm{rebase} : \mathrm{SystemIO}(P_2) \to \mathrm{SystemIO}(P_1).
\]
Equivalently: `SystemIO` is contravariant in presentation translations (rebasing is pullback).
...
- Added by tooling: `SystemIO` packages provers/model-checkers and makes rebasing a one-liner (pullback of certificates), so interoperability is operational, not just semantic.
...
- Tool I/O and rebasing:
  ...
  - `LogOS/Ports/Semantic/BoundarySystemIO.agda` (build/rebase `SystemIO` at the boundary port level)
...
- Kernel-derived canonical ports + bootstrapping:
  ...
  - Signature transport: `LogOS/Kernel/Reindex.agda` and `LogOS/Kernel/LogicKernel/Reindex.agda`.
...
`SystemIO.rebase` gives you one for \(P_1\) by pulling back certificates along
