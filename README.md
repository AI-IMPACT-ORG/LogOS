...
This Agda library, combined with extensive CI checks for vacuity and correctness, as well as with grounding documentation forms an effective development environment for human-AI collaboration to develop formal models of anything. Just download the library, ask you favorite code assistent (Codex, Claude Code, OpenCode, Cursor, etc.) to familiarise itself with the content of the library folders. Start exploring, followed by learning and building.

A word of warning: this entire library as well as its documentation was largely generated by AI and checked by AI. It features extensive CI and a very clear architecture, all implemented to reduce hallucination impact. Some of the nicest results in the library are conditional theorems. It is an open problem if the semantics of the documentation exactly fits the syntax of the code, as the particular applications provided would require a community verified re-axiomatisation of entire scientific fields. This is obviously beyond the scope of the library currently. For more targeted scope models it is strongly recommended to verify results using human expertise.

Top usage tips: Always ensure good coding architecture (clean code, clean architecture for starters) for optimal results. Refactor almost constantly for presentation, and double and triple check results, ideally resetting chatbot memory to prevent cross-memory leakage. Best results are obtained usually with SOTA reasoning models, but these come with a hefty cost of time. 

## Abstract

Models are typically expressed in dedicated domain-specific formal languages, which makes interoperability across domains and communities difficult. We demonstrate a scalable architecture for networks of heterogenous logic systems that targets translations between formal models, leveraging software engineering methods operationalised by AI. This "inter-logics" architecture is inherently self-referential as it treats boundaries of logical systems as logical systems, formalised into ports and adapters and an operating-system analog for "kernels". The architecture distinguishes carefully between equal, equivalent and "equal under assumptions". The latter is an effective definition of "guarded truth" as "stability under resource-constrained interactions". A general transpiler/pass calculus falls out of the same port logic; bootstrapping and CHL compilation are canonical instances.
