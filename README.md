...
# LogOS: AI assisted, machine checked formal models of networks of formal models.

This Agda library forms an effective development environment for human-AI collaboration to develop formal models and heterogeneous formal model networks of basically anything that can be modelled in code. It relies on some pretty fundamental and interesting mathematics, but can already be used today without knowing too much about underlying math. 
...
This entire library as well as its documentation was largely generated by AI and checked by AI - it is a proof of concept of its own use. It features extensive CI, a very clear architecture and an extensive documentation, all implemented in the proving language Agda and aligned to reduce hallucination impact. Some of the nicest results in the library are conditional theorems with specific assumptions. It is an open problem if the semantics of the documentation *exactly* fits the syntax of the code, as some of the particular applications provided would require community verified re-axiomatisation of entire scientific fields. This is left as an exercise to the readers - and one that may be surprisingly easy for true experts. For more targeted scope models it is strongly recommended to verify results using human expertise.
